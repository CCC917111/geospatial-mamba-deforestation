...
Example:
    python -m mamba_forest.evaluate \
        --weights runs/mamba_exp06/best.weights.h5 --test-shards 32
"""
...
from mamba_forest.train import evaluate
...
def parse_args() -> argparse.Namespace:
  ...
  p.add_argument("--test-shards", type=int, default=32)
  p.add_argument("--batch-size", type=int, default=8)
  p.add_argument("--d-model", type=int, default=32,
                 help="Must match the value used for training")
  p.add_argument("--dropout", type=float, default=0.2)
  p.add_argument("--output-dir", default=None,
                 help="Where to write metrics.json (defaults to the weights' directory)")
  return p.parse_args()
...
def main() -> None:
  ...
  forest_f1 = float(np.mean(per_class_f1[list(data_lib.FOREST_CLASS_INDICES)]))

  print(f"accuracy  : {accuracy:.4f}")
  print(f"mean IoU  : {mean_iou:.4f}")
  print(f"macro F1  : {float(np.mean(per_class_f1)):.4f}")
  print(f"forest F1 : {forest_f1:.4f}")
